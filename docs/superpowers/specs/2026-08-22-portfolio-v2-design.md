...
- **Motion (the only motion on the site):**
  ...
  - Nothing else animates. No canvas loops, no rAF — **the ASCII portrait is the one exception** (§7): a pointer-driven rAF loop that runs only while the pointer is over the canvas or while the glyphs are settling back home, and never idles. Disabled under `prefers-reduced-motion: reduce` and on non-hover / coarse-pointer devices, where the portrait stays exactly the single static draw.
...
Static, precomputed, Gazi's algorithm. Data: `lib/ascii/tone_{400,280,220}.json` (particle lists `{c, x, y, a}`; gzip ≈ 5 KB, 5 KB, 3 KB) generated by `scripts/ascii/gazi_ascii.py` from `scripts/ascii/portrait.png` (cutout of the selfie); keep the script and the cutout in the repo so the art can be regenerated. Component `AsciiPortrait` (client): picks the size by breakpoint (Gazi's `calculateSize`), sets `canvas.width = size * dpr` (dpr capped at 2), `ctx.scale(dpr,dpr)`, `font = 7px ui-monospace` (5px at ≤280), `textAlign = "center"`, `textBaseline = "middle"`, and draws every particle **once** at rest in accent at its own alpha (`a * 1.15`, clamped — the owner's brightness lift). Re-draws only on a debounced resize that changes the size bucket.

**Hover (the one exception to §3's "no rAF" rule).** The portrait is the site's only pointer-driven animation. On mount and on every bucket change the particles are flattened into typed arrays (home x/y/alpha + glyph index, plus current x/y/alpha for easing) and every frame repaints with the same `fillText` + `fillStyle = rgba(accent, a)` calls the static draw uses, so the resting frame is byte-identical to the pre-hover build. A pre-rendered glyph atlas + `drawImage` was measured and rejected — blitting a cell to a fractional destination bilinear-resamples the 7px glyph and the portrait comes out visibly soft (8.5% of subpixels changed, peak delta 96/255) — and it buys nothing: the browser caches glyph rasters, so all 1096 `fillText` calls cost **0.76ms/frame at DPR 2**, under 5% of the 16.7ms budget. `pointermove` / `pointerleave` / `pointercancel` on the canvas drive it (the canvas keeps `aria-hidden="true"` and gains pointer events). Glyphs within `R = 90px * size/400` of the cursor are pushed radially outward by up to `14px * size/400` and brightened toward alpha 1.0, both weighted by a smoothstep falloff of `1 - d/R` so there is no hard edge at the radius; each frame eases current → target by 0.15. **The loop only runs while the pointer is over the canvas or while glyphs are still settling** — it stops (and snaps the last hundredth of a pixel to home) once everything is within 0.05px of home and the pointer has left. No idle animation. Gated on `matchMedia("(hover: hover) and (pointer: fine)")` matching **and** `prefers-reduced-motion: reduce` **not** matching; otherwise no listeners are attached and the portrait is the single static draw described above. Listeners and any pending rAF are torn down on unmount and on bucket change. `aria-hidden="true"` on the canvas; a visually-hidden `<span>` "Portrait of Lucas Rocchetti rendered in ASCII" beside it; `<noscript><pre>` fallback from `tone_400.txt` with `letter-spacing: .098em; line-height: 1.1` so the grid isn't squashed. The SSR output reserves the canvas box (width/height set) so there is no layout shift.

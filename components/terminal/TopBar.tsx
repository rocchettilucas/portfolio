      ...
      <div className="bar-inner gap-4 text-[13px] max-sm:gap-3">
        ...
        >
          ...
          <span aria-hidden className="whitespace-nowrap">
            <span className="max-md:hidden">lucas@portfolio:</span>~<span className="prompt">$</span>
          </span>
        </Link>

        {/* Below 768px this is a swipeable strip rather than a wrapped or truncated list. */}
        ...
        {/* Two buttons, one per breakpoint, because the accessible name differs: the wide
            ...
            them is displayed, so only one is in the tab order. */}
        <button
          type="button"
          onClick={openPalette}
          className="shrink-0 whitespace-nowrap text-muted hover:text-fg max-md:hidden"
        >
          press / for commands
        </button>
        <button
          ...
          className="inline-flex h-10 w-10 shrink-0 items-center justify-center text-muted hover:text-fg md:hidden"
        >
